...
parfor iNum = 1:numTest
    ...
    save([dirname,'/',['method',num2str(ik)]],['Variable',num2str(ik)],['allobj',num2str(ik)],['err',num2str(ik)]);
    save([dirname,'/',['method',num2str(ik)]],['S_P',num2str(ik)],['A_P',num2str(ik)],['mean0bj',num2str(ik)],['meanSP',num2str(ik)],'para1','-append');    
...
if flog1
  ...
  %  ylim([1.3*10^-3 2.5*10^-3])
    ...
    close; figure(2)
    plot(T_set,pow2db(MeanSP5*1e3),'--^','LineWidth',2,'MarkerSize',6); hold on
    plot(T_set,pow2db(MeanSP3*1e3),'--V','LineWidth',2,'MarkerSize',6); hold on  
    plot(T_set,pow2db(MeanSP6*1e3),'-.^','LineWidth',2,'MarkerSize',6); hold on
    plot(T_set,pow2db(MeanSP2*1e3),'-->','LineWidth',2,'MarkerSize',6); hold on
    plot(T_set,pow2db(MeanSP1*1e3),'-^','LineWidth',2,'MarkerSize',6); hold on  
    grid on;
    set(gca,'GridLineStyle','--','GridColor','k', 'GridAlpha',0.2);  
    h11 = legend('Static UAV','TO w/o PC','Fly-hover  w/ PC',...
    'BCD-SCA','BCD-ADMM','interpreter','latex');
  ...
  %  ylim([1.3*10^-3 2.5*10^-3])
    xlabel('T (s)','interpreter','latex','FontSize',14);
    ylabel('Sum transmit power (dBm)','interpreter','latex','FontSize',14);
    
 
    saveas(gcf,[dirname1,'/AlogT.fig']);
    print 'AlogT.eps' '-depsc2' '-r600'
  ...
  %%  
    ...
    temperr1 = err1{4};
    h1 = semilogy(temperr1,'-.','LineWidth',2,'Color',RGBmatrix(1,:));hold on
    ...
    load([dirname,'/',['method1']])
    temperr1 = err1{9};
    h1 = semilogy(temperr1,'--','LineWidth',2,'Color',RGBmatrix(2,:));hold on  
    ...
    load([dirname,'/',['method1']])    
    temperr1 = err1{8};
    h1 = semilogy(temperr1,'-','LineWidth',2,'Color',RGBmatrix(6,:));hold on  
    ...
    temperr2 = err2{1};
    h1 = semilogy(temperr2,'-.','LineWidth',1.5,'Color',RGBmatrix(1,:));hold on
    ...
    load([dirname,'/',['method2']]) 
    temperr2 = err2{2};
    h1 = semilogy(temperr2,'--','LineWidth',1.5,'Color',RGBmatrix(2,:));hold on 
    ...
    load([dirname,'/',['method2']])    
    temperr2 = err2{8};
    h1 = semilogy(temperr2,'-','LineWidth',1.5,'Color',RGBmatrix(6,:));hold on
