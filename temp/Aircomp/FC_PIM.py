...
def FC(power_tx, num_device, PCA_dim, PCA_number):
    # parameters
    ...
    data_test_pca_normed[:,PCA_number:] = 0
...
svm_accuracy_list = np.zeros((len(PCA_dim_list), 1))
mlp_accuracy_list = np.zeros((len(PCA_dim_list), 1))
discriminant_gain_list = np.zeros((len(PCA_dim_list), 1))

for i in range(len(PCA_dim_list)):
    PCA_number = int(PCA_dim_list[i])
    svm_accuracy_list[i], mlp_accuracy_list[i], discriminant_gain_list[i], noise, data_noise, data = FC(power_tx, num_device, PCA_dim, PCA_number)

np.save('./save_model/save_results/svm_accuracy_with_PCA_dim_FC.npy', svm_accuracy_list)
np.save('./save_model/save_results/mlp_accuracy_with_PCA_dim_FC.npy', mlp_accuracy_list)
np.save('./save_model/save_results/discriminant_gain_with_PCA_dim_FC.npy', discriminant_gain_list)