...
for i in range(len(PCA_dim_list)):
    PCA_number = int(PCA_dim_list[i])  # PCA dimension, M
    ...
    #var_dist = rng.uniform(0, var_dist_scale, (num_device, PCA_dim))  # variance of distortion, delta_{k,m}
    var_dist = rng.uniform(0, var_dist_scale, (num_device, 1))
    for j in range(1, PCA_dim):
        rng2 = default_rng(j)
        var_dist = np.hstack((var_dist, rng2.uniform(0, var_dist_scale, (num_device, 1))))  # rayleigh fading ~ CN(0,1)
    ...
    # baseline
    f_vec_init = np.ones((num_antenna, 1))  # beamforming init, f
    c_zf_init = 0.4 * np.sqrt(2 * power_tx * channel_gain ** 2 @ (f_vec_init ** 2))
