...
for idx_class_a in range(num_class):
    for idx_class_b in range(idx_class_a):
        for idx_m in range(2):
            alpha_init[int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b, idx_m] = ((mean_class[idx_class_a, idx_m] - mean_class[idx_class_b, idx_m]) ** 2) / ((np.sum(var_dist[:, idx_m].reshape(1,num_device) @ (c_zf_init ** 2)) + var_comm_noise * np.sum(f_vec_init ** 2)) / ((np.sum(c_zf_init)) ** 2) + var_class[idx_m])

disc_init = 2*alpha_init.sum() / num_class / (num_class-1)
print("The discriminant gain before optimization is: {}".format(disc_init))
...
while(diff>1e-2):
    ...
    for idx_class_a in range(num_class):
        for idx_class_b in range(idx_class_a):
            for idx_m in range(2):
                constrains += [(cp.sum_squares(cp.multiply(c_zf, np.sqrt(var_dist[:, idx_m].reshape(-1, 1)))) + var_comm_noise * cp.sum_squares(f_vec)) + (np.sum(c_zf)) ** 2 * var_class[idx_m] - np.sum(c_zf_init) ** 2 * ((mean_class[idx_class_a, idx_m] - mean_class[idx_class_b, idx_m]) ** 2 / alpha_init[int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b, idx_m]) - 2 * (mean_class[idx_class_a, idx_m] - mean_class[idx_class_b, idx_m]) ** 2 * np.sum(c_zf_init) / alpha_init[int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b, idx_m] * cp.sum(c_zf - c_zf_init) + (np.sum(c_zf_init) * (mean_class[idx_class_a, idx_m] - mean_class[idx_class_b, idx_m]) / alpha_init[int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b, idx_m]) ** 2 * (alpha[idx_m][int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b] - alpha_init[int(idx_class_a * (idx_class_a-1) / 2) + idx_class_b, idx_m]) <= 0]
    ...
    print("The discriminant gain after {}-th interation is: {}".format(count, prob.value))
    count += 1
...
#
#
# svm_model_file = './save_model/svm_model_{}dimension.pkl'.format(12)
# mlp_model_file = './save_model/mlp_model_{}dimension.pkl'.format(12)
#
...
#
# data_test_pca_add_noise = add_noise_to_normed_pca(data_test_pca_normed, c_zf_init, f_vec_init)
#
