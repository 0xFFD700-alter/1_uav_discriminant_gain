...
# calculate mean and variance of each feature and class
# num_class = 4
# mean_class = np.zeros((num_class, PCA_dim*3))  # mean of ground-true feature
# var_class = np.zeros((num_class, PCA_dim*3))  # variance of ground-true feature
# num_train_per_class = 800
# num_test_per_class = 200
...
#     var_class[idx_class, :] = np.var(class_data, axis=0)
...
#         data_train_pca_normed[num_train_per_class * idx_class + idx_train_per_class,:] = (data_train_pca[num_train_per_class * idx_class + idx_train_per_class, :] - mean_class[idx_class, :]) / np.sqrt(var_class[idx_class, :]) + mean_class[idx_class, :]
#     for idx_test_per_class in range(num_test_per_class):
#         data_test_pca_normed[num_test_per_class * idx_class:num_test_per_class * (idx_class + 1), :] = (data_test_pca[num_test_per_class * idx_class + idx_test_per_class, :] - mean_class[idx_class, :]) / np.sqrt(var_class[idx_class, :]) + mean_class[idx_class, :]
...
#     var_class[idx_class, :] = np.var(class_data_normed, axis=0)
...
# FLAG_OPT = False
# if FLAG_OPT:
#     power_tx = np.load('./save_model/save_opt_variable/power_tx_opt.npy', allow_pickle=True)
#     power_quantization = np.load('./save_model/save_opt_variable/power_quantization_opt.npy', allow_pickle=True)
#     print('The accuracy after optimization is:')
# else:
#     power_tx = np.load('./save_model/save_opt_variable/power_tx.npy', allow_pickle=True)
#     power_quantization = np.load('./save_model/save_opt_variable/power_quantization.npy', allow_pickle=True)
#     print('The accuracy before optimization is:')


# power_tx_opt_3radar = np.array([0.34705018, 0.37605507])  # P_{r,k}
# power_quantization_opt_3radar = np.array([2.44661637, 6.76459411])  # Q_{k}
# # power_quantization = np.array([8.91098965e+03, 3.08684979e-01, 2.11429403e+13])
#
# # power_tx_opt_3radar = np.array([0.29583048])  # P_{r,k}
# # power_quantization_opt_3radar = np.array([1.09403793])  # Q_{k}
#
# power_tx = power_tx_opt_3radar
# power_quantization = power_quantization_opt_3radar
#
# data_test_pca_add_noise = add_noise_to_normed_pca(data_test_pca_normed, power_tx, power_quantization)
# accuracy_SVM = svm_inference(classifier, data_test_pca_add_noise, label_test)
# accuracy_MLP = mlp_inference(mlp, data_test_pca_add_noise, label_test)
#
# accuracy_SVM_clean = svm_inference(classifier, data_test_pca_normed, label_test)
# accuracy_MLP_clean = mlp_inference(mlp, data_test_pca_normed, label_test)
#
# print(accuracy_SVM, accuracy_MLP)
# print('\n')
# print(accuracy_SVM_clean, accuracy_MLP_clean)
